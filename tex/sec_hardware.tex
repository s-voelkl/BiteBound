...
\begin{itemize}
    % \item Full Redraw: Performed only at round transitions or configuration resets. Static elements such as borders and the complete maze layout are rendered entirely. Horizontally adjacent pixels are drawn as unified spans to minimize SPI transaction overhead.
...
\begin{itemize}
    \item Command Topic (\texttt{mauc2026/group\_03/game/command}):
          Receives control payloads from external dashboards.
    ...
    \item Telemetry Topic (\texttt{mauc2026/group\_03/game/telemetry}):
          Publishes state reports at a 2\,Hz interval.
          The \texttt{JsonBuilder} class structures the combined hardware information, configuration records, game progress, physics parameters, and raw sensor readings into a unified JSON format.
\end{itemize}
