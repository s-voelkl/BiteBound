...
The code operates on a dual-core microcontroller to separate timing-sensitive game updates from network Input/Output (I/O) activities, as illustrated in~\Cref{fig:hardware-architecture}.

\begin{figure}[p]
    \centering
    \includegraphics[width=0.95\linewidth]{../diagrams/out/hardware-architecture/hardware-architecture.png}
...
Transmitting a full 16-bit color frame ($240 \times 280$ pixels) requires writing 131.25 KB of data. Since SPI transfers one bit per clock cycle, a 27 MHz bus provides a theoretical throughput of 27 Mbit/s, resulting in a frame transmission time of approximately 40 ms, derived from the ratio between total frame size and bus bandwidth~\cite{computools2026spi}.
...
Updating a $240 \times 280$ display in 16-bit color (RGB565) requires pushing 131.25 KB of data per frame. Over a standard, stable 27 MHz SPI bus, a full frame transmission takes roughly 40 ms, which physically violates the 20ms constraint of a 50Hz game loop.
...
\begin{figure}[h]
    ...
    \begin{minipage}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=0.8\linewidth]{../assets/esp32-labyrinth.png}
    ...
    \begin{minipage}[b]{0.48\linewidth}
        \centering
        \includegraphics[width=0.8\linewidth]{../assets/esp32-loading.png}
